...
class SamplesSet(list):
    ...
    @classmethod
    def from_directory(cls, instrument, directory_path='samples', start_seconds=0, end_seconds=None, shift_distribution=0.1):
        # naming_by: "midi_number", "nameWithOctave"
        # Todo: logging
        ...
        for file in tqdm(files):
            sample = Sample.from_file(file[:-4], shift_distribution, start_seconds=start_seconds, end_seconds=end_seconds)
            samples_set.append(sample)

        return samples_set

    @classmethod
    def from_midi_file(cls, instrument, samples_name, resonance_seconds=0., shift_distribution=0.1, naming_by="midi_number", write=True):
        # naming_by: "midi_number", "nameWithOctave"
        # Todo: logging
        ...
        for note in tqdm(piece.notes):
            ...
            if naming_by == "midi_number":
                output_name = str(note.note_number) + "_" + str(round(note.duration + resonance_seconds, 3)) + "_" + str(note.velocity)
            elif naming_by == "nameWithOctave":
                output_name = note.pitch.nameWithOctave + "_" + str(round(note.duration + resonance_seconds, 3)) + "_" + str(note.velocity)
            else:
            ...
            sample = Sample(note.velocity, note.note_number, note.start_seconds, note.end_seconds, note_signal, spectrogram_log, time_vector,
...
class Sample(Note):
    def __init__(self, velocity, note_number, start_seconds, end_seconds, signal, spectrogram_log, time_vector, fundamental_bin,
                 partials_bins, partials_amplitudes, partials_distribution, shift_distribution, file_name=None):
    ...
    def from_file(cls, file_name, shift_distribution, start_seconds=0, end_seconds=None, audio_path=SAMPLES_PATH):
        ...
        partials_amplitudes, partials_distribution = Sample.get_partials_info(spectrogram_log, partials_bins, shift_distribution)

        return cls(velocity, note_number, 0, end_seconds, signal_cut, spectrogram_log, time_vector, fundamental_bin, partials_bins,
                   partials_amplitudes, partials_distribution, shift_distribution, file_name=file_name)
...
if __name__ == '__main__':
    ...
    # _samples_set = SamplesSet.from_midi_file(_instrument, _samples_name, resonance_seconds=0.3, shift_distribution=0.1, naming_by="midi_number",
    #                                          write=True)
